...
from data_loader import load_all_data, compute_attribution
...
<style>
    ...
    .section-header { color: #444; font-size: 0.85rem; font-weight: 600;
...
with st.sidebar:
    ...
    all_channels = sorted(touches["channel"].unique().tolist())
    selected_channels = st.multiselect(
        "Channels", all_channels, default=all_channels
    )

    date_min = touches["date_day"].min()
    date_max = touches["date_day"].max()
...
roas             = (total_conversions * 85) / total_spend if total_spend else 0  # $85 avg order

# ── Header ────────────────────────────────────────────────────────────────────
st.title("📊 Multi-Touch Attribution Dashboard")
st.caption(f"Showing **{attribution_model}** attribution · "
           f"{str(start_date)} → {str(end_date)} · "
           f"{len(selected_channels)} channel(s) selected")
...
k5.metric("CAC",              f"${cac:,.0f}",
          f"{int(total_contacts)} customers")
...
with tab1:
    ...
    with col_a:
        st.markdown('<p class="section-header">Daily spend by channel</p>',
                    unsafe_allow_html=True)
        daily = (t_filt.groupby(["date_day", "channel"])["spend_usd"]
                 .sum().reset_index())
        fig = px.area(
    ...
    with col_b:
        st.markdown('<p class="section-header">Spend share by channel</p>',
                    unsafe_allow_html=True)
    ...
    st.markdown('<p class="section-header">Campaign performance table</p>',
                unsafe_allow_html=True)
    camp_table = (
    ...
    camp_table["CTR"]    = (camp_table["clicks"] / camp_table["impressions"]).map("{:.2%}".format)
...
with tab2:
    ...
    with col_c:
        st.markdown(f'<p class="section-header">Revenue attributed by channel — {attribution_model}</p>',
                    unsafe_allow_html=True)
        attr_by_ch = (a_filt.groupby("channel")[model_col]
                      .sum().reset_index()
    ...
    with col_d:
        st.markdown('<p class="section-header">Attribution model comparison</p>',
                    unsafe_allow_html=True)
        comp = (a_filt.groupby("channel")
                .agg(first=("first_touch_credit", "sum"),
    ...
    st.markdown('<p class="section-header">Attribution delta: first-touch vs last-touch</p>',
                unsafe_allow_html=True)
    st.caption("Channels above 0 are over-credited by last-touch vs first-touch. "
               "Channels below 0 drive awareness but lose credit at last-touch.")
    delta = comp.copy()
...
with tab3:
    ...
    with col_e:
        st.markdown('<p class="section-header">Lead funnel by stage</p>',
                    unsafe_allow_html=True)
        stage_order = ["subscriber", "lead", "marketingqualifiedlead",
        ...
        stage_counts = (hubspot["lifecycle_stage"]
                        .value_counts()
    ...
    with col_f:
        st.markdown('<p class="section-header">First-touch channel → customer conversion</p>',
                    unsafe_allow_html=True)
        ch_conv = (hubspot.groupby("first_touch_channel")
                   .agg(total=("contact_id", "count"),
                        customers=("is_customer", "sum"))
                   .reset_index())
        ch_conv["conv_rate"] = ch_conv["customers"] / ch_conv["total"]
    ...
    st.markdown('<p class="section-header">Days to close by first-touch channel</p>',
                unsafe_allow_html=True)
    days_close = (hubspot[hubspot["days_lead_to_close"].notna()]
...
with tab4:
    source = st.selectbox(
        "View raw table",
        ["Unified touchpoints", "fct_attribution", "Google Ads", "Meta Ads",
         "LinkedIn Ads", "HubSpot contacts"],
    )
    table_map = {
